...
for i = 1:N
    radiusHandle(i) = plot(x(1,i), x(2,i), '-.r', 'LineWidth', 2);
end
...
if videoFLag 
    vid = VideoWriter(['sensing_radius_with_', num2str(N), '_robots.mp4'], 'MPEG-4');
    vid.Quality = 100;
    vid.FrameRate = 72;
    open(vid);
...
total_cost = [];
for t = 1:iterations
    if videoFLag && mod(t,20)                               % Record a video frame every 10 iterations
            writeVideo(vid, getframe(gcf)); 
    end
    ...
    [Px, Py] = lloydsAlgorithm(x(1,:)',x(2,:)', crs, verCellHandle, radiusHandle, 70, .3, center, sigma, detSigma);
...
title(['Heighted Lloyds ' num2str(N) '-limited sensing ground robots'])
xlabel('Time')
ylabel('Computed Cost')
grid on;
saveas(gcf, ['sensing_radius_with_' num2str(N) '_bots.png'])
...
function [Px, Py] = lloydsAlgorithm(Px,Py, crs, verCellHandle, radHandle, res, radius, center, sigma, detSigma)
...
%             xrange, 0];
    ...
    for i = 1:numel(c) %calculate the center of mass of each cell
        circX = [circX {Px(i) - radius*cos(0:.1:2*pi)}];
        circY = [circY {Py(i) - radius*sin(0:.1:2*pi)}];
        ...
        in2 = sqrt((Px(i) - coords(:, 1)).^2 + (Py(i) - coords(:, 2)).^2) < radius;
        xArrayIn = coords(in & in2, 1)';
        yArrayIn = coords(in & in2, 2)';
        positionMassSumX = 0;
        positionMassSumY = 0;
        totalMass = 0;
    ...
    for i = 1:numel(c) % update Voronoi cells
        ...
        set(radHandle(i), 'XData', circX{i}, 'YData', circY{i});
    end
